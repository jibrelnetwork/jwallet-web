...
describe('Initial app loading', () => {
  ...
  it('The text on the page matches with the layout', () => {
    ...
    expect(loginTermsPage.checkboxes[0].getText()).toBe('I understand that my funds are stored securely on my personal computer. ' + 'No private data is sent to Jibrel AG servers. All encryption is done locally in browser')
    expect(loginTermsPage.checkboxes[1].getText()).toBe('I consent that the Jwallet service is provided as is without warranty. ' + 'Jibrel AG does not have access to my private information and would not be able to assist in the resolution of issues concerning money loss of any kind')
    expect(loginTermsPage.checkboxes[2].getText()).toBe('I agree to allow cookies to collect anonymous usage data to improve the quality of the Jwallet')
    expect(loginTermsPage.checkboxes[3].getText()).toBe('I have read and accepted the Terms of Use and Privacy Policy')
    expect(loginTermsPage.confirmAndContinueButton.getText()).toBe('Confirm and continue')
  })

  //Check that all checkboxes are clickable
  it('All checkboxes are clickable', () => {
    loginTermsPage.checkboxes.forEach((checkbox, idx) => {
      expect(checkbox.$('input[type=checkbox]').getAttribute('checked')).toBeFalsy(`Checkbox #${idx} value is true`)
      checkbox.$('label').click()
      expect(checkbox.$('input[type=checkbox]').getAttribute('checked')).toBeTruthy(`Checkbox #${idx} value is false`)
    })
  })
  ...
  it('"Terms of Use" and "Privacy Policy" links are clickable', () => {
    ...
    browser.pause(1000)
    expect(browser.getUrl()).toBe('https://jwallet.network/docs/JibrelAG-TermsofUse.pdf')
    browser.closeWindow()
    ...
    browser.switchWindow('jwallet.network/docs')
    browser.pause(1000)
    expect(browser.getUrl()).toBe('https://jwallet.network/docs/JibrelAG-PrivacyPolicy.pdf')
    browser.closeWindow()
    browser.switchToWindow(currentTabId)
    browser.pause(1000)
  })
