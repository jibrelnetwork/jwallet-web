...
describe('Create or Import first wallet', () => {
  beforeAll(
    'Open Login Terms page, accept all conditions, click to create New Wallet',
    () => {
      loginTermsPage.open()
  ...
  it('"Set Payment Password" page checks', () => {
    ...
    expect($('input[name="password"]').getAttribute('placeholder')).toBe(
      'Payment Password'
    )
    //Check 'Confirm Payment Password' input with placeholder
    expect(
      $('input[name="password-confirm"]').getAttribute('placeholder')
    ).toBe('Confirm Payment Password')
    //Check 'Password hint' input with placeholder
    expect($('input[name="password-hint"]').getAttribute('placeholder')).toBe(
      'Password hint'
    )
