...
function* checkFavoriteDataValid(name: string, description: string, address?: string): Saga<void> {
  ...
  if (isAddressInvalid) {
    yield put(favorites.setFormFieldError(
      'address',
      i18n._(
        'Contacts.errors.addressInvalid',
        null,
        { defaults: 'Please input valid address' },
      ),
    ))
  }
  ...
  if (isNameInvalid) {
    yield put(favorites.setFormFieldError(
      'name',
      i18n._(
        'Contacts.errors.nameInvalid',
        null,
        { defaults: 'Name length should be at least 2 symbols' },
      ),
    ))
  }
  ...
  if (isDescriptionInvalid) {
    yield put(
      favorites.setFormFieldError(
        'description',
        i18n._(
          'Contacts.errors.descriptionInvalid',
          null,
          { defaults: 'Description length should be at least 2 symbols' },
        ),
      ),
    )
  }
...
function* checkFavoriteAlreadyExists(items: Favorites, address: string, name: string): Saga<void> {
  ...
  if (foundItem && foundItem.isAddedByUser) {
    yield put(favorites.setFormFieldError(
      'address',
      i18n._(
        'Contacts.errors.addressDuplicate',
        null,
        { defaults: 'Favorite with this address already exists' },
      ),
    ))
  ...
  if (isFavoriteNameExist) {
    yield put(favorites.setFormFieldError(
      'name',
      i18n._(
        'Contacts.errors.nameDuplicate',
        null,
        { defaults: 'Favorite with this name already exists' },
      ),
    ))
  ...
  if (isAddressWalletsNameExist) {
    yield put(favorites.setFormFieldError(
      'name',
      i18n._(
        'Contacts.errors.walletNameDuplicate',
        null,
        { defaults: 'There is wallet with such name' },
      ),
    ))
  ...
  if (isAddressNameExist) {
    yield put(favorites.setFormFieldError(
      'name',
      i18n._(
        'Contacts.errors.aadressNameDuplicate',
        null,
        { defaults: 'There is address with such name' },
      ),
    ))
