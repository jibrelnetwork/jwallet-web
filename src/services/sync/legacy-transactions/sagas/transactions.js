...
import { selectProcessingBlock } from '../selectors/blocks'
import {
  selectCurrentAssets,
...
function* getWalletCreatedBlockNumber(): Saga<void> {
  return yield select(selectCurrentAddressCreatedBlockNumber)
}
...
function* syncProcessingBlockStatus(): Saga<void> {
  try {
    while (true) {
      ...
      const assets = yield select(selectCurrentAssetsList)
...
function* fetchByOwnerRequest(
  ...
  const activeAssets = yield select(selectCurrentAssetsList)
...
function* resyncTransactionsByOwnerAddress(
  ...
  while (true) {
    ...
    const digitalAssets = yield select(selectCurrentAssets)
